...
class SpacecraftSpeedDAO(object):
    ...
    def write_reading(self, spacecraft_name, journey_id, speed, speed_unit, reading_time):
        ...
        insert_future = self._session.execute_async(self.insert_prep_stmt.bind({
            ...
            'speed': speed,
            'pressure_unit': speed_unit,
            'reading_time': reading_time}
        ))

        insert_future.add_callbacks(handle_success, handle_error)

    def get_speed_readings_for_journey(self, spacecraft_name, journey_id, page_size, page_state=None):
        journey_id = UUID('{u}'.format(u=journey_id))

        stmt = BoundStatement(self.select_prep_stmt, fetch_size=int(page_size)).bind({'spacecraft_name': spacecraft_name,
                                                                                      'journey_id': journey_id})
